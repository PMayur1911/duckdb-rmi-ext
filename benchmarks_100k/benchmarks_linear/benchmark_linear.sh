...
run_benchmark() {
    ...
    echo ""
    echo "======================================================="
    echo "[*] Running RMI-linear benchmark for dataset: $NAME"
    echo "======================================================="
...
EOF
    ...
    echo "Baseline Memory: $MEM_BASE KB"
    echo "With RMI Index:  $MEM_FULL KB"
...
EOF
        ...
        if ! output=$($DUCKDB :memory: < tmp_query.sql 2>&1); then
            echo "[WARN] Query $idx failed — marking as miss"
            misses=$((misses+1))
            echo "0" >> "$RESULTS_FILE"
    ...
    echo "[*] Finished RMI-linear benchmark for $NAME"
    echo ""
}
...
run_benchmark \
    ...
    "../outputs_100k/rmi_linear/linear"

run_benchmark \
    "poly_100k" \
    "../insert/insert_data_poly_100k.sql" \
    "../query/query_values_poly_100k.txt" \
    "../outputs_100k/rmi_linear/poly"

run_benchmark \
    "random_100k" \
    "../insert/insert_data_random_100k.sql" \
    "../query/query_values_random_100k.txt" \
    "../outputs_100k/rmi_linear/random"

echo "[*] ALL RMI-linear (100k) benchmarks completed."
