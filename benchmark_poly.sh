...
RESULTS_FILE="results_rmi_poly.txt"
OUTPUT_DIR="query_outputs_rmi_poly"

rm -f "$RESULTS_FILE"
mkdir -p "$OUTPUT_DIR"

echo "[*] Running RMI benchmark (poly model, stable execution)..."

idx=1
while read -r target; do

    SQL_FILE="rmi_poly_query_${idx}.sql"
...
-- Create the RMI index using the poly model
...
echo "[*] Completed RMI polynomial-model benchmark."
